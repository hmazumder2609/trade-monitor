...
export async function handleSystemRequest(
  ...
  try {
    ...
    if (action === 'probe') {
      ...
      return {
        probes: probes.map(r => (r.status === 'fulfilled' ? r.value : { url: '', ok: false })),
      };
    }
...
async function findProcesses(patterns: string[], autoMode: boolean): Promise<unknown> {
  ...
  const cmd = isMac || isLinux ? `ps axo pid,pcpu,pmem,etime,command` : `tasklist /V /FO CSV`;
  ...
  for (const line of lines) {
    ...
    if (autoMode) {
      if (
        ...
        command.includes('/usr/libexec/') ||
        command.includes('WindowServer')
      )
        continue;
...
function scanCursorTerminals(tailLines: number): unknown {
  ...
  const terminals: any[] = [];
  try {
    const workspaces = readdirSync(cursorProjectsDir, { withFileTypes: true }).filter(d =>
      d.isDirectory()
    );
...
async function executePython(code: string): Promise<unknown> {
  ...
  for (const d of dangerous) {
    if (code.includes(d))
      return { success: false, error: `Blocked: "${d}" is not allowed for safety` };
  }
