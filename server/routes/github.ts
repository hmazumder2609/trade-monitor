...
export async function handleGithubRequest(
  ...
  if (action === 'trending') {
    ...
    try {
      ...
      const keywordFilter = searchQuery
        ? `+${encodeURIComponent(searchQuery)}+in:name,description,readme`
        : '';
  ...
  if (action === 'activity') {
    ...
    try {
      const data = await ghFetch(
        '/users/' + (query.username || '') + '/received_events?per_page=30',
        token
      );
...
function describeEvent(e: any): string {
  switch (e.type) {
    ...
    case 'IssueCommentEvent':
      return `commented on #${e.payload?.issue?.number}`;
    case 'ReleaseEvent':
