...
export async function handleOnChainRequest(
  ...
  if (action === 'status') {
    ...
    if (!apiKey) return { status: 'no_key', message: 'No Whale Alert API key configured' };

    try {
      const resp = await fetch(`${WHALE_ALERT_BASE}/status?api_key=${apiKey}`, {
        signal: AbortSignal.timeout(8_000),
      });
      const data = await resp.json();
      return { status: resp.ok ? 'ok' : 'error', details: data };
    } catch (err: any) {
  ...
  if (action === 'transactions') {
    ...
    if (!apiKey) {
      return { transactions: getDemoTransactions(), source: 'demo' };
    }

    try {
      const transactions = await cached('whale:transactions', () => fetchWhaleTransactions(apiKey));
      return { transactions, source: 'whale-alert' };
    } catch (err: any) {
      return { transactions: getDemoTransactions(), source: 'demo', error: err.message };
    }
  }
