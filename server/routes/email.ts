...
export async function handleEmailRequest(
  ...
  const clientSecret =
    (headers['x-gmail-client-secret'] as string) || process.env.GMAIL_CLIENT_SECRET || '';
  const refreshToken =
    (headers['x-gmail-token'] as string) || process.env.GMAIL_REFRESH_TOKEN || '';

  if (!clientId || !clientSecret || !refreshToken) {
    return {
      emails: [],
      configured: false,
      message: 'Gmail OAuth not configured — need Client ID, Client Secret, and Refresh Token',
    };
  }
  ...
  if (!accessToken) {
    return {
      emails: [],
      configured: true,
      error: 'Failed to refresh access token. Check Client ID, Secret, and Refresh Token.',
    };
  }
  ...
  try {
    ...
    return {
      ...
      configured: true,
      totalUnread: listData.resultSizeEstimate || 0,
    };
  } catch (err: any) {
