...
export async function handleFeishuRequest(
  ...
  const appSecret =
    (headers['x-feishu-app-secret'] as string) || process.env.FEISHU_APP_SECRET || '';

  if (!appId || !appSecret) {
    return {
      configured: false,
      error: 'Feishu App credentials not configured',
      chats: [],
      messages: [],
    };
  }
  ...
  if (!token) {
    return {
      configured: true,
      error: 'Failed to get tenant_access_token. Check app_id and app_secret.',
      chats: [],
      messages: [],
    };
  }
        ...
        try {
          ...
          for (const msg of items) {
            ...
            if (!senderName && senderId && senderType === 'user') {
              ...
              try {
                const userResp = await feishuGet(`/contact/v3/users/${senderId}`, token, {
              ...
              if (!senderName) {
                try {
                  const membersResp = await feishuGet(
                    `/im/v1/chats/${chat.chat_id}/members`,
                    token,
                    { member_id_type: 'open_id', page_size: '50' }
                  );
