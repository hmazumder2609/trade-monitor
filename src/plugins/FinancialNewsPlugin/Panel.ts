...
const DEFAULT_SOURCES: NewsSource[] = [
  { id: 'kobeissi', name: 'The Kobeissi Letter', url: 'https://thekobeissiletter.com/blog/rss.xml', enabled: true, type: 'rss' },
  { id: 'yahoo-finance', name: 'Yahoo Finance', url: 'https://finance.yahoo.com/news/rssindex', enabled: true, type: 'rss' },
  { id: 'cnbc', name: 'CNBC Finance', url: 'https://search.cnbc.com/rs/search/combinedcms/view.xml?partnerId=wrss01&id=10000664', enabled: true, type: 'rss' },
  { id: 'bloomberg-markets', name: 'Bloomberg Markets', url: 'https://feeds.bloomberg.com/markets/news.rss', enabled: true, type: 'rss' },
  { id: 'ft-markets', name: 'Financial Times', url: 'https://www.ft.com/markets?format=rss', enabled: false, type: 'rss' },
  { id: 'wsj-markets', name: 'WSJ Markets', url: 'https://feeds.a.dj.com/rss/RSSMarketsMain.xml', enabled: false, type: 'rss' },
  { id: 'zerohedge', name: 'ZeroHedge', url: 'https://feeds.feedburner.com/zerohedge/feed', enabled: false, type: 'rss' },
  { id: 'seeking-alpha', name: 'Seeking Alpha', url: 'https://seekingalpha.com/feed.xml', enabled: false, type: 'rss' },
];
...
const CATEGORY_LABELS: Record<string, string> = {
  tech: 'Tech', finance: 'Finance', world: 'World', ai: 'AI',
  china: 'China', science: 'Science', us: 'US', europe: 'Europe',
};
const ALL_CATEGORIES = Object.keys(CATEGORY_LABELS);

const THREAT_COLORS: Record<ThreatLevel, string> = {
  critical: 'var(--red)', high: 'var(--semantic-high)', medium: 'var(--yellow)',
  low: 'var(--blue)', info: 'var(--text-muted)',
};
const SOURCE_COLORS: Record<string, string> = {
  BBC: '#3b82f6', Reuters: '#ff8800', AP: '#44aa44', 'Al Jazeera': '#9333ea',
};
...
export class FinancialNewsPanel extends Panel {
  ...
  constructor() {
    super({ id: 'financial-news', title: 'Financial News', showCount: true, className: 'panel-wide' });
  ...
  private renderSourceManager(): void {
    ...
    this.sourceManagerEl.querySelectorAll<HTMLInputElement>('input[type="checkbox"]').forEach(cb => {
    ...
    this.sourceManagerEl.querySelector('#finnewsAddBtn')?.addEventListener('click', () => {
      const name = (this.sourceManagerEl!.querySelector('#finnewsNewName') as HTMLInputElement).value.trim();
      const url = (this.sourceManagerEl!.querySelector('#finnewsNewUrl') as HTMLInputElement).value.trim();
      if (!name || !url) return;
      const sources = loadSources();
  ...
  private renderArticleList(): void {
    ...
    if (filtered.length === 0) {
      this.listEl.innerHTML = '<div class="panel-empty">No articles match the current filters.</div>';
      return;
    }
