...
export class CodeStatusPanel extends Panel {
  ...
  private renderTabs(): void {
    ...
    const tabs: { id: CodingTab; label: string }[] = [
      ...
      { id: 'ci', label: '\u2699 CI/CD' },
      { id: 'repos', label: '\u{1F4E6} My Repos' },
      { id: 'activity', label: '\u{1F4E1} Activity' },
    ];
      ...
      <a href="${r.url}" target="_blank" class="ch-trending-row" rel="noopener">
        ...
        <div class="ch-trending-info">
          ...
          <div class="ch-trending-meta">
            ...
            <span>\u{1F374} ${this.fmtNum(r.forks)}</span>
  ...
  private async loadTracked(): Promise<void> {
    ...
    if (repos.length === 0) {
      this.bodyEl.innerHTML = '<div class="panel-empty">Could not fetch repo data</div>';
      return;
    }
  ...
  private async loadCI(): Promise<void> {
    ...
    if (repos.length === 0) {
      this.bodyEl.innerHTML = '<div class="panel-empty">No repos configured.</div>';
      return;
    }

    const resp = await fetch(`/api/github?action=runs&repos=${repos.join(',')}`, {
    ...
    if (runs.length === 0) {
      this.bodyEl.innerHTML = '<div class="panel-empty">No workflow runs</div>';
      return;
    }
  ...
  private async loadMyRepos(): Promise<void> {
    ...
    const token = getSecret('GITHUB_PAT');
    if (!token) {
      this.bodyEl.innerHTML = '<div class="panel-empty">GitHub PAT not configured.</div>';
      return;
    }
    ...
    this.setCount(repos.length);
    if (repos.length === 0) {
      this.bodyEl.innerHTML = '<div class="panel-empty">No repos</div>';
      return;
    }
  ...
  private async loadActivity(): Promise<void> {
    ...
    try {
      const meResp = await fetch('https://api.github.com/user', {
        headers: { Authorization: `Bearer ${token}`, 'User-Agent': 'MDM/1.0' },
      });
      const me = (await meResp.json()) as any;
      if (!me.login) {
        this.bodyEl.innerHTML = '<div class="panel-empty">Could not get username</div>';
        return;
      }

      const resp = await fetch(`/api/github?action=activity&username=${me.login}`, {
        headers: { 'X-Github-Token': token },
      });
      const data = await resp.json();
      ...
      if (events.length === 0) {
        this.bodyEl.innerHTML = '<div class="panel-empty">No recent activity</div>';
        return;
      }
