...
export function getMarketStatus(now = new Date()): MarketStatus {
  ...
  if (t >= 240 && t < 570)
    return { label: 'PRE-MKT', color: 'text-[hsl(38,95%,55%)]', pulse: true };
  if (t >= 570 && t < 960) return { label: 'MKT OPEN', color: 'text-up', pulse: true };
  if (t >= 960 && t < 1200)
    return { label: 'AFTER-HRS', color: 'text-[hsl(38,95%,55%)]', pulse: true };
  return { label: 'CLOSED', color: 'text-muted-foreground', pulse: false };
}

export function shouldIgnoreCommandShortcut(
