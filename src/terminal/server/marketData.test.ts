...
test('parseStooqCurrent parses current quote csv rows', () => {
  const current = parseStooqCurrent(
    'AAPL.US,20260316,210017,252.105,253.885,249.88,252.82,32074210,'
  );
...
test('parseStooqCurrent rejects non-csv payloads', () => {
  assert.throws(
    () => parseStooqCurrent('</html>'),
    /No current quote data|Malformed current quote row/
  );
});
...
test('parseStooqHistory normalizes unavailable volume to zero', () => {
  const bars = parseStooqHistory(
    ['Date,Open,High,Low,Close,Volume', '2026-03-13,17.24,17.24,17.24,17.24,N/D'].join('\n')
  );
