...
export async function registerRoutes(httpServer: Server, app: Express): Promise<void> {
  ...
  app.get(
    ...
    handleFinance(async () => getIndexSparklines())
  );
  ...
  app.get(
    ...
    handleFinance(async req => {
      ...
      return quotes.map(
        ({ symbol, price, change, changePercent, quoteSource, isLive, status }) => ({
          symbol,
          price,
          change,
          changePercent,
  ...
  app.get(
    '/api/finance/gainers',
    handleFinance(async () => getMarketMovers('gainers'))
  );
  app.get(
    '/api/finance/losers',
    handleFinance(async () => getMarketMovers('losers'))
  );
  app.get(
    '/api/finance/active',
    handleFinance(async () => getMarketMovers('active'))
  );
  app.get(
    '/api/finance/sentiment',
    handleFinance(async () => getMarketSentiment())
  );
  ...
  app.get(
    ...
    handleFinance(async req => {
      ...
      const feedProvider =
        typeof req.query.feedProvider === 'string' ? req.query.feedProvider : undefined;
  ...
  app.get(
    ...
    handleFinance(async () => getEconomicsSnapshot())
  );
  app.get(
    '/api/finance/economics/calendar',
    handleFinance(async () => getEconomicCalendar())
  );
  ...
  app.post('/api/finance/portfolio-analytics', async (req, res) => {
    ...
    try {
      ...
      const historyEntries = await Promise.all(
        symbols.map(
          async symbol =>
            [
              symbol,
              (await getOHLCV(symbol, '1Y', '1d')).map(point => ({
                date: point.date,
                close: point.close,
              })),
            ] as const
      ...
      const benchmark = (await getOHLCV('SPY', '1Y', '1d')).map(point => ({
        date: point.date,
        close: point.close,
      }));
