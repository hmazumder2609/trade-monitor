...
const TRACKED_RELEASES: Array<{
  ...
  {
    pattern: /advance monthly sales for retail and food services/i,
    category: 'consumption',
    importance: 'medium',
  },
  {
    pattern: /industrial production and capacity utilization/i,
    category: 'activity',
    importance: 'medium',
  },
...
export function parseFredReleaseSchedule(
  html: string,
  releaseId: number
): EconomicReleaseScheduleDate[] {
...
export function filterUpcomingReleaseDates(dates: EconomicReleaseScheduleDate[], now = new Date()) {
  ...
  return dates
    ...
    .filter(
      (entry, index, all) =>
        all.findIndex(
          candidate => candidate.date === entry.date && candidate.timeCt === entry.timeCt
        ) === index
    )
    .sort((left, right) => {
      if (left.date !== right.date) return left.date.localeCompare(right.date);
...
export function parseFredReleaseDetail(
  html: string,
  releaseId: number
): Omit<EconomicEventDetail, 'upcomingDates'> {
  const heading = stripTags(html.match(/<h1>([\s\S]*?)<\/h1>/i)?.[1] ?? '');
  const titleTag = stripTags(html.match(/<title>([\s\S]*?)<\/title>/i)?.[1] ?? '').replace(
    /\s*\|\s*FRED.*$/i,
    ''
  );
  const title = heading || titleTag || `Release ${releaseId}`;
  const meta = classifyRelease(title) ?? {
    category: 'activity' as EconomicEventCategory,
    importance: 'medium' as EconomicEventImportance,
  };

  const breadcrumbMatches = Array.from(
    html.matchAll(/<a[^>]+class="breadcrumb_link"[^>]+href="([^"]+)"[^>]*>([^<]+)<\/a>/gi)
  );
  const source = breadcrumbMatches.at(-1);

  const releaseCalendarUrl =
    toAbsoluteFredUrl(
      html.match(
        /<a[^>]+href="([^"]*\/releases\/calendar\?rid=[^"]+)"[^>]*>\s*Release Calendar\s*<\/a>/i
      )?.[1]
    ) ?? `${FRED_BASE_URL}/releases/calendar?rid=${releaseId}&y=${new Date().getUTCFullYear()}`;
  const releaseWebsiteUrl = toAbsoluteFredUrl(
    html.match(/<a[^>]+href="([^"]+)"[^>]*>\s*Release Website\s*<\/a>/i)?.[1]
  );

  const tables = Array.from(
    html.matchAll(
      /<a[^>]+href="([^"]*\/release\/tables\?rid=[^"]+)"[^>]*>([^<]+)<\/a>&nbsp;\s*<span[^>]*>(?:[^<]*\()?([\d,]+)(?:\)?[^<]*)<\/span>/gi
    )
  )
    .slice(0, 6)
    .map(match => ({
...
export async function getEconomicEventDetail(
  releaseId: number,
  now = new Date()
): Promise<EconomicEventDetail> {
