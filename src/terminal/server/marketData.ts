...
const GENERAL_NEWS_FEEDS: RssFeedConfig[] = [
  ...
  {
    url: buildGoogleNewsSearchUrl(
      'stock market OR federal reserve OR earnings OR inflation OR bitcoin when:2d'
    ),
...
function buildSnapshotFromBars(
  ...
  const high = Number.isFinite(overrides.high)
    ? Number(overrides.high)
    : Math.max(...bars.map(bar => bar.high), close);
  const low = Number.isFinite(overrides.low)
    ? Number(overrides.low)
    : Math.min(...bars.map(bar => bar.low), close);
...
export function buildQuoteFromSnapshot({
  symbol,
  provider,
  profile,
  current,
  ...
  const marketCap =
    profile?.marketCap && profile.referencePrice
  ...
  return items
    .map((item): NewsItem | null => {
      ...
      const publishedAt = new Date(
        String(item?.pubDate ?? item?.published ?? item?.updated ?? Date.now())
      ).toISOString();
...
async function getCoinGeckoQuotes(symbols: string[]): Promise<Map<string, Quote>> {
  ...
  try {
      ...
      if (!profile || !data) {
        results.set(
          ...
          setCached(
            quoteCache,
            `quote:${symbol}`,
            buildReferenceFallbackQuote(symbol),
            QUOTE_TTL_MS
          )
        );
        continue;
    ...
    for (const symbol of uncached) {
      results.set(
        symbol,
        setCached(quoteCache, `quote:${symbol}`, buildReferenceFallbackQuote(symbol), QUOTE_TTL_MS)
      );
    }
  }
  ...
  try {
    ...
    const quote = buildQuoteFromSnapshot({
      ...
      current: buildSnapshotFromBars(sessionBars, {
        close: Number.isFinite(meta?.regularMarketPrice)
          ? Number(meta?.regularMarketPrice)
          : undefined,
        high: Number.isFinite(meta?.regularMarketDayHigh)
          ? Number(meta?.regularMarketDayHigh)
          : undefined,
        low: Number.isFinite(meta?.regularMarketDayLow)
...
export async function getOHLCVSeries(
  ...
  return {
    ...
    status: quote.status,
    supportsIntraday:
      getProfile(upper)?.assetClass === 'crypto' || quote.status.freshness === 'current',
  };
}
...
export async function getMarketSentiment() {
  ...
  const score = Math.round(
    Math.max(0, Math.min(100, 50 + ((positive - negative) / news.length) * 50))
  );
...
async function fetchNewsFeeds(cacheKey: string, feeds: RssFeedConfig[]) {
  ...
  const normalized = uniqueBy(items, item => item.url || item.title).sort(
