 ...
 * Works with any OpenRouter-compatible model (default: Claude Opus 4.6 Free).
 */
 ...
 */
export async function summarizeFeishuMessages(
  messages: Array<{ chatName: string; senderName: string; content: string }>
): Promise<string | null> {
  if (!getPreferences().aiSummaryEnabled) return null;
  if (messages.length === 0) return null;

  const content = messages
    .map((m, i) => `${i + 1}. [${m.chatName}] ${m.senderName}: ${m.content}`)
    .join('\n');
...
export async function generateDailyBriefing(data: {
  ...
  if (data.stocks.length > 0) {
    const movers = data.stocks.filter(
      s => s.changePercent != null && Math.abs(s.changePercent) > 1
    );
    if (movers.length > 0) {
      sections.push(
        'MARKET MOVERS:\n' +
          movers
            .map(
              s =>
                `- ${s.symbol}: ${s.changePercent! >= 0 ? '+' : ''}${s.changePercent!.toFixed(1)}%`
            )
            .join('\n')
      );
    }
