/**
 * TradingPanel — Launch surface for the Bloomberg-style terminal (blmtrm).
 *
 * The full trading experience lives in the terminal sub-app. This panel is the
 * bridge: it shows a summary of what's inside and opens the terminal with the
 * active symbol pre-loaded.
 *
 * Dev:  terminal runs on http://localhost:5000
 * Prod: terminal is served at /terminal by the same Express server
 */
import { Panel } from './Panel';
import { getStockSymbols } from '@/services/settings-store';

// Detect dev vs prod: if we're on localhost the terminal runs on its own port.
...
export class TradingPanel extends Panel {
  ...
  async refresh(): Promise<void> {
    // Re-render in case watchlist changed
    this.render();
  }
