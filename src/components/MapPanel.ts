/**
 * MapPanel — interactive world map with data overlays and live flight tracking.
 *
 * Data layers:
 *  ✈ Live flights (OpenSky Network) — searchable, toggleable
 *  🔵 News sources with active articles
 *  🔴 Critical/high-threat news alerts (pulsing)
 *  🟢 Your location / calendar events
 *  🟢/🔴 Server probes
 *
 * Flight overlay panel lets you search callsigns and toggle flight visibility.
 */
...
export class MapPanel extends Panel {
  ...
  private trackedFlights = new Set<string>(); // icao24 ids of selected flights
  ...
  private wireFlightOverlayEvents(overlay: HTMLElement): void {
    ...
    overlay.querySelectorAll('.mfo-check').forEach(cb => {
      cb.addEventListener('change', e => {
        ...
        this.updateFlightMarkers();
        // Update tracked count without full re-render
    ...
    // Click row to show flight detail popup on map
  ...
  private async fetchFlights(): Promise<void> {
    try {
      ...
      // Update markers for tracked flights (positions change over time)
  ...
  private updateFlightMarkers(): void {
    ...
    // Only show tracked flights (or all if selectAll is on)
    const visible = this.selectAllFlights
      ? this.flights.slice(0, 200) // cap for performance
  ...
  public setMarkers(markers: MapMarker[]): void {
    // Preserve flight markers when other markers are set
